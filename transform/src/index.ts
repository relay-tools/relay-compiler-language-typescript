...
function visitor(ctx: ts.TransformationContext, sf: ts.SourceFile, opts: Options): ts.Visitor {
...
export function transformer(opts?: Options) {
  return (context: ts.TransformationContext): ts.Transformer<ts.SourceFile> => {
    return (sf: ts.SourceFile): ts.SourceFile => ts.visitNode(sf, visitor(context, sf, opts || {}));
  }
}
