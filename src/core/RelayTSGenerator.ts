...
export function generate(node: Root | Fragment, options: Options): string {
	...
	const resultFile = ts.createSourceFile("grapghql-def.ts", "", ts.ScriptTarget.Latest, /*setParentNodes*/ false, ts.ScriptKind.TS);
...
function makeProp(
	...
	let { key, schemaName, value, conditional, nodeType, nodeSelections } = selection;
	if (nodeType) {
...
const hasTypenameSelection = (selections: Selection[]) => selections.some(isTypenameSelection);
const onlySelectsTypename = (selections: Selection[]) => selections.every(isTypenameSelection);

function selectionsToAST(selections: Selection[][], state: State, refTypeName?: string): ts.TypeNode {
	...
	if (
		...
		const otherPropWithComment = ts.addSyntheticLeadingComment(otherProp, ts.SyntaxKind.MultiLineCommentTrivia,
...
function exactObjectTypeAnnotation(properties: ts.PropertySignature[]): ts.TypeLiteralNode {
...
function mergeSelection(a: Selection | null | undefined, b: Selection): Selection {
	if (!a) {
		return {
			...b,
			conditional: true,
	...
	return ts.createImportDeclaration(
		...
		ts.createImportClause(
			...
			ts.createNamedImports(
				names.map(name => ts.createImportSpecifier(undefined, ts.createIdentifier(name)),
				)
			),
		),
	...
	return {
		leave: {
			...
			Fragment(node: any) {
				...
				const numConecreteSelections = flattenedSelections.filter(s => s.concreteType)
					.length;
	...
	return exportType(
		...
		exactObjectTypeAnnotation(
			node.argumentDefinitions.map(arg => {
				return readOnlyObjectTypeProperty(arg.name, transformInputType(arg.type, state), !(arg.type instanceof GraphQLNonNull));
			}),
		),
	);
}
