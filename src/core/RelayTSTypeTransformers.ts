...
function transformGraphQLScalarType(type: GraphQLScalarType, state: State): ts.TypeNode {
...
function transformGraphQLEnumType(type: GraphQLEnumType, state: State): ts.TypeNode {
	state.usedEnums[type.name] = type;
	return ts.createTypeReferenceNode(ts.createIdentifier(type.name), []);
}

export function transformInputType(type: GraphQLInputType, state: State): ts.TypeNode {
	if (type instanceof GraphQLNonNull) {
...
function transformNonNullableInputType(type: GraphQLInputType, state: State) {
	if (type instanceof GraphQLList) {
		...
		const props = Object.keys(fields)
			...
			.map(field => {
				const property = ts.createPropertySignature(
					...
					field.type instanceof GraphQLNonNull ? ts.createToken(ts.SyntaxKind.QuestionToken) : undefined,
		...
		throw new Error(`Could not convert from GraphQL type ${(type as GraphQLInputType).toString()}`);
	}
}
